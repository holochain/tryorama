...
module.exports = (scenario, tape) => {

scenario('delete_post', run => new Promise(resolve => {
  tape('delete_post', t => {
    run(async (s, { alice, bob }) => {
      //create post
...
scenario('post max content size 280 characters', run => new Promise(resolve => {
  tape('post max content size 280 characters', t => {
    run(async (s, insts) => {
      console.debug('>>> 0')
      const { alice } = insts
